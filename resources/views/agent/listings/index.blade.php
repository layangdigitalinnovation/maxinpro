...
@section('title', 'Kelola Listing Saya — MaxinPro')
...
<div class="flex flex-col lg:flex-row justify-between lg:items-center gap-4 mb-6">
    ...
    <div class="flex flex-wrap gap-2 w-full lg:w-auto items-center justify-start lg:justify-end">
        ...
        <a href="{{ route('agent.listings.create') }}" class="whitespace-nowrap h-11 px-4 sm:px-5 inline-flex items-center rounded-lg bg-brand-blue text-white text-[13px] font-extrabold hover:bg-blue-700">
            + Tambah Listing
        </a>
    </div>
</div>
...
<div class="bg-white border border-brand-line rounded-2xl overflow-hidden">
    <div class="overflow-x-auto"><table class="w-full min-w-[800px] text-left border-collapse">
        ...
        <tbody class="divide-y divide-brand-line">
            @forelse($listings as $listing)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="p-4 text-sm text-center">
                        <label class="custom-toggle">
                            <input type="checkbox" class="publish-toggle-input" 
                                ...
                                data-id="{{ $listing->id }}"
                                data-url="{{ route('agent.listings.update-publish-ajax', $listing) }}"
                            >
                    ...
                    <td class="p-4 text-sm text-center">
                        <div class="inline-flex items-center gap-2">
                            <input type="number" min="0"
                                ...
                                data-id="{{ $listing->id }}"
                                data-url="{{ route('agent.listings.update-order-ajax', $listing) }}"
                            >
                    ...
                    <td class="p-4 text-sm flex gap-3">
                        <a href="{{ route('agent.listings.edit', $listing) }}" class="text-blue-600 hover:underline font-bold">Edit</a>
                        <form action="{{ route('agent.listings.destroy', $listing) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus listing ini?');">
                            @csrf @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="7" class="p-6 text-center text-gray-500 text-sm">Belum ada listing yang ditambahkan.</td>
                </tr>
            @endforelse
        </tbody>
    </table></div>
