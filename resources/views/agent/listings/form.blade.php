...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('agent.listings.index') }}" class="h-11 px-6 inline-flex items-center rounded-lg bg-gray-200 text-brand-navy text-[13px] font-extrabold hover:bg-gray-300">
        Kembali
    </a>
</div>
...
<form action="{{ $listing->exists ? route('agent.listings.update', $listing) : route('agent.listings.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @if($listing->exists)
        @method('PUT')
    @endif
