...
<article class="max-w-[760px] mx-auto px-8 pt-11">
    <span class="inline-block px-3 py-1 bg-brand-blue/10 text-brand-blue text-[11px] font-black uppercase rounded-full mb-3">{{ $article->category ?? 'Berita' }}</span>
    <h1 class="text-brand-navy text-[28px] min-[900px]:text-[34px] leading-[1.25] font-black mt-2 mb-4">{{ $article->title }}</h1>
    ...
    <div class="flex items-center gap-3 mb-8">
        ...
        <div>
            ...
            <p class="text-[#7a8399] text-[12px] font-bold"><time datetime="{{ $article->published_at?->toDateString() }}">{{ $article->published_at?->translatedFormat('d M Y') }}</time></p>
        </div>
    </div>
    ...
    <div class="py-8 flex flex-col min-[600px]:flex-row items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            <a href="https://wa.me/?text={{ urlencode($article->title . ' - ' . route('articles.show', $article->slug)) }}" target="_blank" rel="noopener" class="w-11 h-11 rounded-full flex items-center justify-center transition-colors shadow-sm share-wa" title="Share ke WhatsApp">
            ...
            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('articles.show', $article->slug)) }}" target="_blank" rel="noopener" class="w-11 h-11 rounded-full flex items-center justify-center transition-colors shadow-sm share-fb" title="Share ke Facebook">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            </a>
            <a href="https://twitter.com/intent/tweet?url={{ urlencode(route('articles.show', $article->slug)) }}&text={{ urlencode($article->title) }}" target="_blank" rel="noopener" class="w-11 h-11 rounded-full flex items-center justify-center transition-colors shadow-sm share-x" title="Share ke X (Twitter)">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </a>
            <button onclick="navigator.clipboard.writeText('{{ route('articles.show', $article->slug) }}'); alert('Link berhasil disalin!')" class="w-11 h-11 rounded-full border flex items-center justify-center transition-colors shadow-sm share-copy" title="Salin Link">
