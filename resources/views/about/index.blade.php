...
@section('title', 'Tentang Kami — MaxinPro')
@section('meta_description', 'MaxinPro adalah partner properti terpercaya Anda di kawasan Jabodetabek, menyederhanakan proses jual, beli, dan KPR.')
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-5">
        @foreach ([
            ...
            <div class="bg-white rounded-3xl p-8 text-center shadow-[0_8px_30px_rgba(0,0,0,0.06)] border border-brand-line/50 transition-transform duration-300 hover:-translate-y-1">
                ...
                <div class="text-[#5e6a84] text-[13px] font-extrabold uppercase tracking-wide">{{ $stat['label'] }}</div>
            </div>
        @endforeach
    </div>
</section>
                ...
                <ul class="space-y-3">
                    @foreach ([
                        ...
                        <li class="flex items-start gap-3">
                            ...
                            <span class="text-[#4a5568] text-[14.5px] font-medium leading-relaxed">{{ $mission }}</span>
                        </li>
                    @endforeach
                </ul>
            </div>
