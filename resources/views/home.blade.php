...
@section('title', 'MaxinPro — Temukan Properti Impianmu')
@section('meta_description', 'Cari rumah, apartemen, ruko, dan tanah dijual di BSD, Bintaro, Alam Sutera, Gading Serpong, dan Jakarta.')
...
<section class="relative bg-white pt-10 min-[900px]:pt-16 pb-32">
    ...
    <div class="absolute inset-0 z-0">
        <img src="{{ asset('images/hero-skyline.png') }}" alt="" class="w-full h-full object-cover object-center">
    ...
    <div class="relative max-w-[1100px] mx-auto px-4 z-10 flex flex-col min-[900px]:flex-row items-center">
        <div class="w-full min-[900px]:w-1/2 pt-10 min-[900px]:pt-24 pb-20">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                <a href="{{ route('listings.index') }}" class="inline-flex justify-center items-center h-12 px-6 rounded-full bg-brand-blue text-white font-extrabold text-[14px] whitespace-nowrap w-full sm:w-auto shadow-lg shadow-brand-blue/30 hover:scale-105 transition-transform">
                    Jelajahi Listing →
                </a>
                <a href="{{ route('titip-properti.create') }}" class="inline-flex justify-center items-center h-12 px-6 rounded-full bg-white border border-brand-line text-brand-navy font-extrabold text-[14px] whitespace-nowrap w-full sm:w-auto shadow-sm hover:bg-gray-50 transition-colors">
...
<div class="relative max-w-[1100px] mx-auto px-4 -mt-32 min-[900px]:-mt-48 mb-10 z-20">
    <div class="bg-white rounded-[28px] shadow-[0_20px_40px_rgba(0,0,0,0.08)] p-4 min-[900px]:p-6">
        ...
        <form id="search-form" action="{{ route('listings.index') }}" method="GET">
            ...
            <div class="flex items-center gap-3 flex-wrap mt-5 px-2">
                ...
                @foreach ($popularAreas as $area)
                    <a href="{{ route('listings.index', ['q' => $area->name]) }}" class="text-[12px] font-extrabold px-3.5 h-8 inline-flex items-center rounded-full bg-[#f4f7fb] text-[#3a455e] hover:bg-brand-soft hover:text-brand-blue transition-colors">{{ $area->name }}</a>
                @endforeach
            </div>
        </form>
    </div>
...
<section class="max-w-[1100px] mx-auto px-4 mt-12 mb-16">
    <div class="grid grid-cols-2 min-[600px]:grid-cols-3 min-[900px]:grid-cols-6 gap-4">
        <a href="{{ route('listings.index') }}" class="border border-brand-line rounded-3xl p-5 text-center group hover:border-brand-blue hover:shadow-lg transition-all bg-white">
        ...
        <a href="{{ route('projects.index') }}" class="border border-brand-line rounded-3xl p-5 text-center group hover:border-brand-blue hover:shadow-lg transition-all bg-white">
        ...
        <a href="{{ route('titip-properti.create') }}" class="border border-brand-line rounded-3xl p-5 text-center group hover:border-brand-blue hover:shadow-lg transition-all bg-white">
        ...
        <a href="{{ route('kpr.index') }}" class="border border-brand-line rounded-3xl p-5 text-center group hover:border-brand-blue hover:shadow-lg transition-all bg-white">
        ...
        <a href="{{ route('about.index') }}" class="border border-brand-line rounded-3xl p-5 text-center group hover:border-brand-blue hover:shadow-lg transition-all bg-white">
...
<section class="max-w-[1280px] mx-auto px-8 mb-16 relative">
    <div class="relative group">
        ...
        <!-- Promo 1 -->
        <a href="{{ route('kpr.index') }}" class="snap-start shrink-0 w-[85vw] min-[900px]:w-[calc(33.333%-11px)] hover:scale-[1.02] transition-transform">
            <img src="{{ asset('images/promo-kpr.png') }}" alt="Promo KPR Spesial" class="w-full h-auto rounded-2xl shadow-sm border border-brand-line/50">
        </a>

        <!-- Promo 2 -->
        <a href="{{ route('listings.index') }}" class="snap-start shrink-0 w-[85vw] min-[900px]:w-[calc(33.333%-11px)] hover:scale-[1.02] transition-transform">
            <img src="{{ asset('images/cashback.png') }}" alt="Cashback Hingga 50 Juta" class="w-full h-auto rounded-2xl shadow-sm border border-brand-line/50">
        </a>

        <!-- Promo 3 -->
        <a href="{{ route('titip-properti.create') }}" class="snap-start shrink-0 w-[85vw] min-[900px]:w-[calc(33.333%-11px)] hover:scale-[1.02] transition-transform">
            <img src="{{ asset('images/titip-properti.png') }}" alt="Titip Properti Jadi Lebih Mudah" class="w-full h-auto rounded-2xl shadow-sm border border-brand-line/50">
        </a>
...
{{-- PROJECT TERBARU CAROUSEL --}}
<section class="max-w-[1280px] mx-auto px-8 mb-16 relative">
    <div class="flex items-center justify-between mb-4">
        <h2 class="text-brand-navy font-black text-[22px] min-[900px]:text-[24px]">Project Terbaru</h2>
        <a href="{{ route('projects.index') }}" class="text-[13px] font-extrabold text-brand-blue flex items-center gap-1 hover:underline">Lihat Semua →</a>
    </div>
        ...
        <div class="carousel-container flex gap-5 overflow-x-auto snap-x snap-mandatory pb-4 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
            @forelse ($projects as $project)
                <div class="snap-start shrink-0 w-[280px] min-[900px]:w-[300px]">
                    <x-project-card :project="$project" :compact="true" />
                </div>
            @empty
...
<section class="max-w-[1280px] mx-auto px-8 mb-16 relative">
    <div class="flex items-center justify-between mb-4">
        ...
        <a href="{{ route('listings.index') }}" class="text-[13px] font-extrabold text-brand-blue flex items-center gap-1 hover:underline">Lihat Semua →</a>
    </div>
...
<section class="max-w-[1280px] mx-auto px-8 mb-16">
    ...
    <div class="grid grid-cols-2 min-[700px]:grid-cols-3 min-[1000px]:grid-cols-6 gap-4">
        @foreach ($popularAreas as $area)
            <a href="{{ route('area-landing.show', $area) }}" class="relative rounded-[20px] overflow-hidden aspect-square group shadow-sm">
                <img src="{{ $area->image_path ? asset('storage/'.$area->image_path) : asset('images/placeholder-property.jpg') }}" alt="{{ $area->name }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
                <div class="absolute inset-0 bg-gradient-to-t from-brand-navy/90 to-transparent opacity-80"></div>
                <div class="absolute bottom-4 left-4 text-white">
                    <strong class="block text-[14px] font-black mb-0.5">{{ $area->name }}</strong>
                    <span class="text-[11.5px] font-extrabold text-white/80">{{ $area->property_count }}+ Properti</span>
                </div>
            </a>
        @endforeach
    </div>
...
<section class="max-w-[1280px] mx-auto px-8 mb-16 relative">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        @forelse ($articles as $article)
            <a href="{{ route('articles.show', $article->slug) }}" class="bg-white border border-brand-line rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all group flex flex-col h-full">
                <div class="aspect-[1.6/1] relative bg-brand-soft overflow-hidden shrink-0">
                    <img src="{{ $article->cover_image ? asset('storage/' . $article->cover_image) : asset('images/placeholder-property.jpg') }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt="{{ $article->title }}">
                    <div class="absolute left-3 top-3 h-6 px-3 bg-white/90 backdrop-blur rounded-full flex items-center text-[10px] font-black text-brand-blue uppercase shadow-sm">{{ $article->category ?? 'Berita' }}</div>
                </div>
                <div class="p-4 flex flex-col flex-1">
                    <h3 class="text-[15px] font-black text-brand-navy leading-snug mb-2 group-hover:text-brand-blue transition-colors line-clamp-2">{{ $article->title }}</h3>
                    <div class="text-[12px] font-bold text-[#7a8399] mt-auto">{{ $article->published_at ? $article->published_at->translatedFormat('d M Y') : $article->created_at->translatedFormat('d M Y') }}</div>
                </div>
            </a>
        @empty
                    ...
                    <div class="bg-[#f8fafc] rounded-3xl p-6 relative overflow-hidden group hover:shadow-md transition-shadow border border-brand-line/50 w-full flex flex-col">
                        ...
                        <div class="flex items-center gap-4 mb-4 relative z-10">
                            <div class="w-14 h-14 rounded-full bg-brand-navy overflow-hidden shrink-0 border-2 border-white shadow-sm">
                                <img src="{{ $testi->photo_path ? asset('storage/' . $testi->photo_path) : 'https://ui-avatars.com/api/?name='.urlencode($testi->name).'&background=random' }}" alt="{{ $testi->name }}" class="w-full h-full object-cover">
                            </div>
                            <div>
                                <h4 class="text-[15px] font-black text-brand-navy">{{ $testi->name }}</h4>
                                <p class="text-[11.5px] font-bold text-[#7a8399] mb-1">{{ $testi->city }}</p>
                                <div class="flex text-[#f59e0b] text-[12px]">
                                    @for($i = 0; $i < $testi->rating; $i++)★@endfor
                                </div>
                            </div>
                        </div>
                        <p class="text-[13px] font-bold text-[#55617a] leading-relaxed relative z-10 flex-1">{{ $testi->quote }}</p>
                    </div>
                </div>
            @empty
