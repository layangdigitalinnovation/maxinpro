    ...
    <div class="bg-white rounded-[28px] shadow-[0_20px_40px_rgba(0,0,0,0.08)] p-4 min-[900px]:p-6">
        ...
        <form id="search-form" action="{{ route('listings.index') }}" method="GET">
            ...
            <div class="flex items-center gap-3 flex-wrap mt-5 px-2">
                ...
                @foreach (['BSD City', 'Bintaro', 'Alam Sutera', 'Gading Serpong', 'Jakarta Selatan'] as $chip)
                    <a href="{{ route('listings.index', ['q' => $chip]) }}" class="text-[12px] font-extrabold px-3.5 h-8 inline-flex items-center rounded-full bg-[#f4f7fb] text-[#3a455e] hover:bg-brand-soft hover:text-brand-blue transition-colors">{{ $chip }}</a>
                @endforeach
            </div>
        </form>
    </div>
