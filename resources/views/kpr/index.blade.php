...
@section('title', 'Kalkulator KPR — MaxinPro')
@section('meta_description', 'Hitung estimasi cicilan KPR bulanan secara gratis. Lihat syarat pengajuan lengkap dan daftar bank rekanan MaxinPro sebelum mengajukan kredit rumah.')
    ...
    <div class="flex items-center justify-center gap-4 flex-wrap max-w-4xl mx-auto">
        @foreach ($banks as $bank)
            <div class="px-6 py-3 bg-white border border-brand-line/50 rounded-xl shadow-sm hover:shadow-md hover:-translate-y-0.5 hover:border-brand-blue/30 transition-all cursor-default">
                <span class="text-brand-navy text-[14px] font-extrabold tracking-wide">{{ $bank }}</span>
            </div>
        @endforeach
    </div>
</section>
            ...
            <ul class="space-y-4 relative z-10">
                @foreach ($personalDocs as $doc)
                    <li class="flex items-start gap-3">
                        ...
                        <span class="text-[#3a455e] text-[14px] font-medium leading-relaxed">{{ $doc }}</span>
                    </li>
                @endforeach
            </ul>
        </div>
