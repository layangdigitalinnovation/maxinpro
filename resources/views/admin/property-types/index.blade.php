...
@section('title', 'Tipe Properti — MaxinPro')
...
<div class="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6">
    ...
    <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
        ...
        <div class="flex gap-3 w-full sm:w-auto"><a href="{{ route('admin.property-types.create') }}" class="h-11 px-6 inline-flex items-center rounded-lg bg-brand-blue text-white text-[13px] font-extrabold hover:bg-blue-700">
        ...
        <tbody class="divide-y divide-brand-line">
            @forelse($propertyTypes as $type)
                <tr class="hover:bg-gray-50">
                    <td class="p-4 text-sm font-bold text-brand-navy">{{ $type->name }}</td>
                    <td class="p-4 text-sm flex gap-3">
                        <a href="{{ route('admin.property-types.edit', $type) }}" class="text-blue-600 hover:underline font-bold">Edit</a>
                        <form action="{{ route('admin.property-types.destroy', $type) }}" method="POST" onsubmit="return confirm('Hapus tipe properti ini?');">
                            @csrf @method('DELETE')
...
<div class="mt-6">{{ $propertyTypes->links() }}</div>
@endif
@endsection
