...
@section('title', 'Data Developer — MaxinPro')
...
<div class="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6">
    ...
    <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
        ...
        <div class="flex gap-3 w-full sm:w-auto"><a href="{{ route('admin.developers.create') }}" class="h-11 px-6 inline-flex items-center rounded-lg bg-brand-blue text-white text-[13px] font-extrabold hover:bg-blue-700">
        + Tambah Developer
    </a></div>
    </div>
</div>
        ...
        <tbody class="divide-y divide-brand-line">
            @forelse($developers as $developer)
                <tr class="hover:bg-gray-50">
                    <td class="p-4 text-sm font-bold text-brand-navy">{{ $developer->name }}</td>
                    <td class="p-4 text-sm flex gap-3">
                        <a href="{{ route('admin.developers.edit', $developer) }}" class="text-blue-600 hover:underline font-bold">Edit</a>
                        <form action="{{ route('admin.developers.destroy', $developer) }}" method="POST" onsubmit="return confirm('Hapus developer ini?');">
                            @csrf @method('DELETE')
...
<div class="mt-6">{{ $developers->links() }}</div>
@endif
@endsection
