...
@section('title', 'Kelola Listing — MaxinPro')

@section('content')
<div class="flex flex-col lg:flex-row justify-between lg:items-center gap-4 mb-6">
    
    <h1 class="text-brand-navy text-[22px] font-black shrink-0">Kelola Listing</h1>
    <div class="flex flex-wrap gap-2 w-full lg:w-auto items-center justify-start lg:justify-end">
        ...
        <a href="{{ route('admin.listings.trashed') }}" class="whitespace-nowrap h-11 px-4 inline-flex items-center rounded-lg bg-gray-200 text-brand-navy text-[13px] font-extrabold hover:bg-gray-300">
            Sampah
        </a>
        <a href="{{ route('admin.listings.create') }}" class="whitespace-nowrap h-11 px-4 sm:px-5 inline-flex items-center rounded-lg bg-brand-blue text-white text-[13px] font-extrabold hover:bg-blue-700">
            + Tambah Listing
        </a>
    </div>
</div>
        ...
        <tbody class="divide-y divide-brand-line">
            @forelse($listings as $listing)
                <tr class="hover:bg-gray-50">
                    <td class="p-4">
                        <div class="font-bold text-sm text-brand-navy">{{ $listing->title }}</div>
                        <div class="text-xs text-gray-500 mt-1">Oleh: {{ $listing->agent->name ?? 'Admin' }}</div>
                    </td>
                    ...
                    <td class="p-4 text-sm text-center">
                        <label class="custom-toggle">
                            <input type="checkbox" class="publish-toggle-input" 
                                {{ $listing->is_published ? 'checked' : '' }}
                                data-id="{{ $listing->id }}"
                                data-url="{{ route('admin.listings.update-publish-ajax', $listing) }}"
                            >
                    ...
                    <td class="p-4 text-sm text-center">
                        <div class="inline-flex items-center gap-2">
                            <input type="number" min="0"
                                ...
                                value="{{ $listing->sort_order }}" 
                                data-id="{{ $listing->id }}"
                                data-url="{{ route('admin.listings.update-order-ajax', $listing) }}"
                            >
                            <span class="text-green-500 opacity-0 transition-opacity duration-300 save-indicator-{{ $listing->id }}">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                            </span>
                        </div>
                    </td>
                    <td class="p-4 text-sm flex gap-3">
                        <a href="{{ route('admin.listings.edit', $listing) }}" class="text-blue-600 hover:underline font-bold">Edit</a>
                        <form action="{{ route('admin.listings.destroy', $listing) }}" method="POST" onsubmit="return confirm('Pindahkan listing ini ke sampah?');">
                            @csrf @method('DELETE')
