...
@section('title', 'Dashboard Admin — MaxinPro')
    ...
    @foreach ([
        ...
        <div class="bg-white border border-brand-line p-6 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
            <div class="flex items-start justify-between">
                <div>
                    <div class="text-brand-muted text-[13px] font-bold uppercase tracking-wider mb-2">{{ $s['label'] }}</div>
                    <div class="text-brand-navy text-[32px] font-black leading-none">{{ number_format($s['value']) }}</div>
                </div>
                <div class="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl {{ $s['color'] }} shadow-sm">
                    {{ $s['icon'] }}
                </div>
            </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    <div class="bg-white border border-brand-line p-6 flex flex-col">
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('admin.leads.index') }}" class="text-xs font-bold text-brand-blue hover:text-brand-blue2 bg-brand-soft px-3 py-1.5 rounded-lg transition-colors">Lihat Semua</a>
        </div>
                ...
                @foreach($recentLeads as $lead)
                    <li class="flex items-center gap-4 group">
                        ...
                        <div class="flex-1 min-w-0">
                            <span class="block font-bold text-[14px] text-brand-navy truncate group-hover:text-brand-blue transition-colors">{{ $lead->name }}</span>
                            <span class="block text-[12px] text-brand-muted truncate">{{ $lead->property_type ?? 'Properti' }} &bull; {{ $lead->city ?? 'Tanpa Kota' }}</span>
                        </div>
            ...
            @foreach($leadsByStatus as $status => $count)
                <li class="flex justify-between items-center bg-brand-soft/50 hover:bg-brand-soft p-4 rounded-xl border border-transparent hover:border-brand-line/50 transition-colors">
                    <div class="flex items-center gap-3">
                        <div class="w-2 h-2 rounded-full {{ $status === 'new' ? 'bg-orange-500' : ($status === 'contacted' ? 'bg-blue-500' : 'bg-emerald-500') }}"></div>
                        <span class="text-[14px] font-bold text-brand-navy capitalize">{{ $status }}</span>
                    </div>
                    <span class="font-black text-[18px] text-brand-navy bg-white shadow-sm px-3 py-1 rounded-lg border border-brand-line/50">{{ number_format($count) }}</span>
                </li>
            @endforeach
        </ul>
    </div>
