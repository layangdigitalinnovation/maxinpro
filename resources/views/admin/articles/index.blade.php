...
@section('title', 'Data Artikel — MaxinPro')
...
<div class="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6">
    ...
    <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
        ...
        <div class="flex gap-3 w-full sm:w-auto"><a href="{{ route('admin.articles.create') }}" class="h-11 px-6 inline-flex items-center rounded-lg bg-brand-blue text-white text-[13px] font-extrabold hover:bg-blue-700">
        + Tambah Artikel
    </a></div>
    </div>
</div>
        ...
        <tbody class="divide-y divide-brand-line">
            @forelse($articles as $article)
                <tr class="hover:bg-gray-50">
                    <td class="p-4 text-sm font-bold text-brand-navy">{{ $article->title }}</td>
                    <td class="p-4 text-sm">{{ $article->category ?? '-' }}</td>
                    <td class="p-4 text-sm text-gray-500"><div class="line-clamp-1">{{ $article->excerpt ?? '-' }}</div></td>
                    <td class="p-4 text-sm flex gap-3">
                        <a href="{{ route('admin.articles.edit', $article) }}" class="text-blue-600 hover:underline font-bold">Edit</a>
