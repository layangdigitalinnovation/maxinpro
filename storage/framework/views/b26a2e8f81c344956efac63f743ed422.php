...
<form action="<?php echo e($area->exists ? route('admin.areas.update', $area) : route('admin.areas.store')); ?>" method="POST" enctype="multipart/form-data" class="bg-white border border-brand-line rounded-2xl p-6 space-y-6 max-w-2xl">
    <?php echo csrf_field(); ?>
    <?php if($area->exists): ?>
    ...
    <div>
        ...
        <div class="relative w-full overflow-hidden group cursor-pointer area-dropzone" id="dropzone">
            ...
            <!-- Default Placeholder -->
            <div class="absolute inset-0 flex flex-col items-center justify-center p-4 text-center pointer-events-none transition-opacity duration-300" id="upload-placeholder" style="<?php echo e($area->image_path ? 'opacity: 0;' : 'opacity: 1;'); ?>">
            ...
            <!-- Preview Image -->
            <div class="absolute inset-0 pointer-events-none transition-opacity duration-300 bg-black/5" id="image-preview-container" style="<?php echo e($area->image_path ? 'opacity: 1;' : 'opacity: 0;'); ?>">
                <img src="<?php echo e($area->image_path ? asset('storage/' . $area->image_path) : ''); ?>" id="image-preview" class="w-full h-full object-cover" alt="Preview">
