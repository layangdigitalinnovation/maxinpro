<?php $__env->startSection('title', $project->name . ' — ' . $project->area->name . ' | MaxinPro'); ?>
<?php $__env->startSection('meta_description', Str::limit(strip_tags($project->description ?: ('Project ' . $project->name . ' oleh ' . $project->developer->name . ' di ' . $project->area->name . '. Harga mulai ' . $project->formatted_price_from . '.')), 155)); ?>
<?php $__env->startSection('og_type', 'product'); ?>
<?php $__env->startSection('og_image', $project->cover_image ? asset('storage/' . $project->cover_image) : asset('images/og-default.jpg')); ?>
...
<div class="max-w-[1280px] mx-auto px-8 pt-6 flex items-center gap-1.5 text-[#6d7890] text-[12.5px] font-bold">
    ...
    <a href="<?php echo e(route('projects.index')); ?>" class="text-[#6d7890]">Project</a><span>/</span>
    <span class="text-brand-navy"><?php echo e($project->name); ?></span>
</div>
...
<div class="max-w-[1280px] mx-auto px-8 mt-4 grid grid-cols-1 min-[900px]:grid-cols-[1.6fr_1fr] gap-8 items-start">
    <div>
        <div class="rounded-2xl overflow-hidden aspect-[1.8/1] bg-brand-soft mb-5 relative">
            <img src="<?php echo e($project->cover_image ? asset('storage/'.$project->cover_image) : asset('images/placeholder-property.jpg')); ?>" alt="<?php echo e($project->name); ?>" class="w-full h-full object-cover cursor-pointer hover:opacity-90 transition-opacity lightbox-trigger">
            <?php if($project->youtube_url): ?>
                <a href="<?php echo e($project->youtube_url); ?>" target="_blank" class="absolute bottom-4 right-4 bg-white hover:bg-gray-50 text-brand-navy font-extrabold text-[13px] py-2 px-4 rounded-lg shadow-md flex items-center gap-2 transition-colors">
        ...
        <?php if($project->images->isNotEmpty()): ?>
        ...
        <h1 class="text-brand-navy text-[24px] font-black mb-1"><?php echo e($project->name); ?></h1>
        <p class="text-[#55617a] text-[13px] font-bold mb-4"><?php echo e($project->area->name); ?>, <?php echo e($project->area->city); ?></p>

        <div class="flex items-center gap-5 text-[#39445e] text-[13px] font-bold mb-6 flex-wrap">
            <span>By <?php echo e($project->developer->name); ?></span>
            <span><?php echo e($project->status); ?></span>
            <?php if($project->propertyType): ?>
                <span><?php echo e($project->propertyType->name); ?></span>
            <?php endif; ?>
        </div>
    ...
    <aside class="border border-brand-line rounded-2xl p-6 sticky top-[92px]">
        <?php if($project->units_available): ?>
            <span class="inline-block px-3 py-1 bg-green-50 text-green-600 font-extrabold text-[11px] uppercase tracking-wider rounded-lg mb-4">Tersedia Sisa <?php echo e($project->units_available); ?> Unit</span>
        <?php endif; ?>
        
        <div class="text-[#7a8399] text-[12.5px] font-bold mb-1">Harga Mulai Dari</div>
        <div class="text-brand-navy text-[26px] font-black mb-4"><?php echo e($project->formatted_price_from); ?></div>
        ...
        <a href="https://wa.me/<?php echo e(setting('whatsapp_number', '6281112345678')); ?>?text=<?php echo e(urlencode('Halo, saya tertarik dengan project: ' . $project->name)); ?>" target="_blank" rel="noopener"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Layang\maxinpro.com\resources\views\projects\show.blade.php ENDPATH**/ ?>